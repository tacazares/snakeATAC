...
for root, dirs, files in os.walk(args.fastq_dir):
	for file in files:
		if file.endswith("fastq.gz"):
			...
			m = re.search(r"([A-Z0-9a-z]{4})-[0-9A-Z]{2}-.+_([12]).fastq.gz", file)
			if m:
				sample = m.group(1)
...
for root, dirs, files in os.walk("/data/miraldiLab/databank/maxatac/analysis/Barski_CD4_reseq/fastq_subsample"):
	for file in files:
		if file.endswith("fastq.gz"):
			full_path = join(root, file)
			#R1 will be forward reads, R2 will be reverse reads
			m = glob.glob("{sample}_1.fastq.gz", file)
			if m:
				sample = m.group(1)
				# R1 or R2 for forward and reverse read
				reads = m.group(2)  
